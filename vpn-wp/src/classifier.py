from dataloader import VPNDataSet
...
def trainRandomForest():
    # 创建随机森林模型
    model = RandomForestClassifier(n_estimators=100, criterion='gini', max_depth=15, max_features=38)
    ...
    for i in range(epoch):
        model = trainStage(model, X_train, y_train, X_valid, y_valid)
